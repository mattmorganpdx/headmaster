...
> **Headmaster is a simple, private way to modify HTTP request headers — add,
> edit, or remove them per site, with no profiles.**
>
> Add custom request headers, override existing values, or remove headers on the
> URLs you choose. Other header editors lock you into a single "active profile,"
> so sending the same header with different values across environments is
> awkward. Headmaster drops that model — every rule is independent and always
> evaluated, so you can send:
>
